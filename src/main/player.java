...
class player
{
	...
	player(String name, float posx, float posy, float velx, float vely, float r, float g, float b, int numNodes,
			int numLayers)
	{
		br = new Brain(numLayers, numNodes, inputNodes, outputNodes, Functions.p.width, Functions.p.height / 2,
				xPadding, yPadding, 1);
		this.name = name;
		this.posx = posx;
		this.posy = posy;
	...
	player(player old, String newName)
	{
		...
		this.velx = old.velx;
		this.vely = old.vely;
		this.r = Functions.clamp(0, 255, old.r + (int) Functions.p.random(-2, 2));
		this.g = Functions.clamp(0, 255, old.g + (int) Functions.p.random(-2, 2));
		this.b = Functions.clamp(0, 255, old.b + (int) Functions.p.random(-2, 2));
	...
	boolean shouldJump()
	{
		if (br.getOutput()[0].val >= 0.5)
			return true; // if the output node is >= 0.5 jump
		else
			return false;
	}
	...
	void doWork() // created from example code
	{
		...
		// If on the ground and "jump" key is pressed set my upward velocity to the jump
		// speed!
